...
DirPath = r'H:\业务中台\手写笔迹\0714素材\10~60Hz测试结果\60Hz'
Title = 'ly'

def readData():
    alllist = []
    singrecord = []
    data = xlrd.open_workbook(os.path.join(DirPath, 'ly_long.xlsx'))
    sheet = data.sheets()[0]
...
def writeData(alllist):
    ...
    workbook = xlsxwriter.Workbook(os.path.join(DirPath, Title + '相似度准确率.xlsx'))
