...
PATH = r'H:\ocr本地自动化保存结果\OCRresult\Chinese\OCR_resources_Path.txt'
...
def get_ocr_result(file_name):
    ...
    try:
        r = requests.request("POST", url=url, files=body, timeout=100)
...
def get_ruike_result(img_path):
    url = "http://ocr-xtc.eebbk.net/ocr/api/v1.0"
    post_data = {'img_type': 1}
    with open(img_path, 'rb') as f:
        read_byte = f.read()
    image_file_obj = {'upl_img': read_byte}

    response = requests.post(url, data=post_data, files=image_file_obj)
    ...
    res = ''
    tesxtcontent = response.json()['data']['regions']
    for line in tesxtcontent[0]['lines']:
        res += line['text']
    res.replace("“", "\"")
...
def accuracy_test(resoucePath, savePath, compare_mode='c'):
    ...
    filename = '{0}{1}{2}'.format('ocrResult_Chinese_', str(dt), '.xls')
    ...
    for dir in dirList:
        for img_path in os.listdir(dir):
            if img_path.endswith(".jpg") or img_path.endswith(".png") or img_path.endswith(".bmp"):
                ...
                ocr_word = get_ruike_result(abs_img_path).replace("“", "\"").replace("”", "\"").replace(",",
                                                                                                        "，").replace(
                    "（", "(").replace("）", ")").replace("？", "?")
